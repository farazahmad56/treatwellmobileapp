...
export class PatientAttachmentPage {
    ...
    public uploadImage() {
        // Destination URL
        var baseUrl = 'http://104.154.204.71/treatwell_mobile/';
    ...
    displayFullImage(obj: any) {
        this.storage.get('loggedInPatientId').then((val) => {
            if (val !== '') {
                this.photoViewer.show('http://104.154.204.71/patientMobileAttachments/' + val + '/' + obj.FILE_NME, obj.FILE_NME, {share: false});
            }
        });
    }
